...
def partition_domain(act_frac_sys, frac_set_vec, tolerance_intersect, number_partitions_x, number_partitions_y):
    """
    ...
    # Array where all information is stored to in the end form frac_set_vec_list.
    part_frac_set_vec = np.zeros(tot_new_fracs)
    ...
    for ii_frac in part_frac_sys:
        ...
        # Add frac_set_vec values
        part_frac_set_vec[old_index:new_index] = np.ones(new_index - old_index) * frac_set_vec[fracs_to_part[ii]]
    ...
    frac_set_vec = np.delete(frac_set_vec, fracs_to_part, axis=0)

    frac_set_vec = np.hstack((frac_set_vec, part_frac_set_vec))
    ...
    for p in range(0, num_subdomains):
        ...
        frac_set_vec_list.append(frac_set_vec[indices])

    return act_frac_sys_list, frac_set_vec_list, partitions
